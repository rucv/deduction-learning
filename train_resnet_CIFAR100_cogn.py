...
parser.add_argument('--lr-decay-milestones', nargs='+', type=int, default=[40,80,120],
...
#parser.add_argument('--save-dir', default='/users/wenchi/ghwwc/local-loss/results/local-error', type=str,            ### wenchi 10/19/2019
parser.add_argument('--save-dir', default='/users/wenchi/ghwwc/opposite_learning/results/local_error', type=str,         ### wenchi 01/08/2019
...
parser.add_argument('--opls_rt', default=0.5, type=float, help='opposite loss ratio')              ### wenchi 01/30/2020
...
if args.dataset == 'MNIST':
    ...
    opls_rt_min = 0.001
    opls_rt_max = 0.98
    prob_sum_1 = 0             ### wenchi 03/09/2020
    prob_sum_2 = 0             ### wenchi 03/09/2020
...
elif args.model == 'resnet101_cogn':
...
def train(epoch, lr):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        ### In the CIFAR100 case #######
        out = torch.zeros(target.size())
        target_oppo = out.long().cuda()
        pos_oppo_matrix = torch.ones(output.size()).cuda()                          ### wenchi 03/10/2020
        for i in range(target.size()[0]):
            pos = target[i]
            if pos in dict0:
               temp = torch.zeros(1).long().random_(0, m1+m2+m3+m4+m5+m6+m7)
               dict_rem = dict1 + dict2 + dict3 + dict4 + dict5 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict0:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m0
               if pos_oppo in dict1:
                  for k in dict1:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m1
               elif pos_oppo in dict2:
                  for k in dict2:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m2
               elif pos_oppo in dict3:
                  for k in dict3:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m3
               elif pos_oppo in dict4:
                  for k in dict4:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m4
               elif pos_oppo in dict5:
                  for k in dict5:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m5
               elif pos_oppo in dict6:
                  for k in dict6:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m6
               elif pos_oppo in dict7:
                  for k in dict7:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m7

            elif pos in dict1:
               temp = torch.zeros(1).long().random_(0, m0+m2+m3+m4+m5+m6+m7)
               dict_rem = dict0 + dict2 + dict3 + dict4 + dict5 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict1:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m1
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               elif pos_oppo in dict2:
                  for k in dict2:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m2
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m3+m4+m5+m6+m7)
               dict_rem = dict0 + dict1 + dict3 + dict4 + dict5 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict2:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m2
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               elif pos_oppo in dict1:
                  for k in dict1:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m1
               elif pos_oppo in dict3:
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m4+m5+m6+m7)
               dict_rem = dict0 + dict1 + dict2 + dict4 + dict5 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict3:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m3
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m5+m6+m7)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict5 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict4:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m4
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m4+m6+m7)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict4 + dict6 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict5:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m5
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m4+m5+m7)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict4 + dict5 + dict7
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict6:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m6
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
               ...
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m4+m5+m6)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict4 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

               for j in dict7:
                   accum1 += output[i][j]
               prob_sum_1 = accum1/m7
               if pos_oppo in dict0:
                  for k in dict0:
                      accum2 += output[i][k]
                  prob_sum_2 = accum2/m0
            ...
            if prob_sum_1 / prob_sum_2 >= 1:                                                                ### wenchi 03/09/2020
               opls_rt = 0.001
            else:
               opls_rt = 0.98
            pos_oppo_matrix[i][pos_oppo] = opls_rt
        output = output * pos_oppo_matrix

        #pdb.set_trace()
        loss_oppo = F.cross_entropy(1-output, target_oppo)                                                                             ### wenchi 03/09/2020
        loss = loss + loss_oppo
        #loss = loss + args.opls_rt * loss_oppo
        #################################
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    if args.save_dir is not '':
        ...
        dirname = os.path.join(dirname, '{}_{}x{}_{}_{}_dimdec{}_alpha{}_beta{}_bs{}_cpb{}_drop{}{}_bn{}_{}_wd{}_bp{}_detach{}_lr{:.2e}_lrdecay{:.2e}_oplsmin{}_oplsmax{}'.format(
                ...
                int(not args.no_batch_norm), args.optim, args.weight_decay, int(args.backprop), int(not args.no_detach), args.lr, args.lr_decay_fact, opls_rt_min, opls_rt_max))
