...
if args.dataset == 'MNIST':
    ...
    dict2 = [0,5,9,10,12,16,19,20,22,25,28,39,40,41,53,57,61,69,84,86,87,94]      #"man-made"#            ### wenchi 01/27/2020
    dict3 = [8,13,48,58,81,85,89,90]        #"transportation"#
    #dict4 = [0,53,57]                       #"food"#
    dict4 = [47,51,52,54,56,59,62,70,82,83,92,96]        #"plants"#
    dict5 = [17,37,68,76]                   #"building"#
    dict6 = [23,33,49,60,71]                #"nature"#                                      
    m0 = len(dict0)
    m1 = len(dict1)
    m2 = len(dict2)
    m3 = len(dict3)
...
elif args.model == 'wresnet28-10_v1':
...
def train(epoch, lr):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for i in range(target.size()[0]):
            pos = target[i]
            if pos in dict0:
               temp = torch.zeros(1).long().random_(0, m1+m2+m3+m4+m5+m6)
               dict_rem = dict1 + dict2 + dict3 + dict4 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

            elif pos in dict1:
               temp = torch.zeros(1).long().random_(0, m0+m2+m3+m4+m5+m6)
               dict_rem = dict0 + dict2 + dict3 + dict4 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo


            elif pos in dict2:
               temp = torch.zeros(1).long().random_(0, m0+m1+m3+m4+m5+m6)
               dict_rem = dict0 + dict1 + dict3 + dict4 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

            elif pos in dict3:
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m4+m5+m6)
               dict_rem = dict0 + dict1 + dict2 + dict4 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

            elif pos in dict4:
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m5+m6)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict5 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

            elif pos in dict5:
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m4+m6)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict4 + dict6
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

            elif pos in dict6:
               temp = torch.zeros(1).long().random_(0, m0+m1+m2+m3+m4+m5)
               dict_rem = dict0 + dict1 + dict2 + dict3 + dict4 + dict5
               pos_oppo = dict_rem[temp]
               target_oppo[i] = pos_oppo

        #pdb.set_trace()
        loss_oppo = F.cross_entropy(1-output, target_oppo)
...
for epoch in range(start_epoch, args.epochs + 1):
    ...
    if args.save_dir is not '':
        ...
        dirname = os.path.join(dirname, '{}_{}x{}_{}_{}_dimdec{}_alpha{}_beta{}_bs{}_cpb{}_drop{}{}_bn{}_{}_wd{}_bp{}_detach{}_lr{:.2e}_lrdecay{:.2e}_opls{}'.format(
                ...
                int(not args.no_batch_norm), args.optim, args.weight_decay, int(args.backprop), int(not args.no_detach), args.lr, args.lr_decay_fact, args.opls_rt))
